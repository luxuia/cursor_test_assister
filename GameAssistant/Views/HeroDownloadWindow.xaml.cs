...
namespace GameAssistant.Views
{
    ...
    public partial class HeroDownloadWindow : Window
    {
        ...
        private async void LoadButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            StatusText.Text = "正在从 Liquipedia 网页加载...";
            try
            {
                if (type == "英雄")
                {
                    ...
                    StatusText.Text = $"已从 Portal:Heroes 加载 {_heroListItems.Count} 个英雄（中英）";
                    SaveFetchedHeroesToJson(heroes);
                }
                else if (type == "物品")
                {
                    var progress = new Progress<string>(m => StatusText.Text = m);
                    ...
                    StatusText.Text = $"已从网页加载 {_itemListItems.Count} 个物品（中英）";
                    SaveFetchedItemsToJson(items);
                }
                else // 技能
                {
                    var progress = new Progress<string>(m => StatusText.Text = m);
            ...
            catch (Exception ex)
            {
                ...
                MessageBox.Show($"从网页加载失败: {ex.Message}", "错误", MessageBoxButton.OK, MessageBoxImage.Error);
            }
            finally
            {
        ...
        /// <summary>
        /// 物品中文：从 Dota2Items_Complete.json / Dota2Items.json 合并；先按 id 匹配，再按英文 name 回退。
        /// </summary>
        private static void MergeChineseNamesForItems(List<ItemInfo> items)
        {
            try
            {
                var baseDir = AppDomain.CurrentDomain.BaseDirectory;
                var dataDir = Path.Combine(baseDir, "Data");
                var curData = Path.Combine(Directory.GetCurrentDirectory(), "Data");
        ...
        /// <summary>
        /// 技能中文：优先 Dota2Abilities_Complete.json，再 Dota2Abilities.json。支持 Pascal 键（Abilities/Id/NameCn）。
        /// </summary>
        ...
        /// </summary>
        private static void SaveFetchedHeroesToJson(List<HeroInfo> heroes)
        {
            try
            {
                ...
                var doc = new
                {
                    sourceUrl = "https://liquipedia.net/dota2/Portal:Heroes",
                    fetchedAt = DateTime.UtcNow.ToString("o"),
                    game = "Dota2",
                    count = heroes.Count,
                    description = "英雄 id/name/nameCn/attribute/iconUrl 中英双版对应关系，从 Portal:Heroes 拉取",
        ...
        /// </summary>
        private static void SaveFetchedItemsToJson(List<ItemInfo> items)
        {
            try
            {
                string dataDir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Data");
                ...
                var doc = new
                {
                    sourceUrl = "https://liquipedia.net/dota2/Portal:Items",
                    fetchedAt = DateTime.UtcNow.ToString("o"),
                    game = "Dota2",
                    count = items.Count,
                    description = "物品 id/name/nameCn/cost/iconUrl 中英双版对应关系，从网页拉取",
